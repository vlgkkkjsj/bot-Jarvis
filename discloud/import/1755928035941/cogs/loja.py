...
class NickModal(discord.ui.Modal, title="✏️ Alterar Apelido"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        print(f"[DEBUG] {membro} ({membro.id}) pediu para mudar apelido para '{novo_nick}' no servidor '{guild.name}'")
        print(f"[DEBUG] Bot: {bot_member} ({bot_member.id}) | Cargo do bot: {bot_member.top_role.position} | Cargo do membro: {membro.top_role.position}")
        print(f"[DEBUG] Permissão 'manage_nicknames': {guild.me.guild_permissions.manage_nicknames}")
        ...
        try:
            ...
            if after_nick != novo_nick:
                print("[WARN] Edit normal falhou, forçando requisição direta à API do Discord...")
                await guild._state.http.edit_member(guild.id, membro.id, nick=novo_nick, reason="Forçado (fallback API)")

            await interaction.response.send_message(
                f"✅ O apelido de {membro.mention} foi alterado para **{novo_nick}**!",
                ephemeral=True
            )
            ...
            await interaction.response.send_message(
                "⛔ O Discord recusou a alteração (sem permissão suficiente).",
                ephemeral=True
            )

        except discord.HTTPException as e:
            await interaction.response.send_message(
                f"❌ Erro HTTP do Discord: `{e}`",
                ephemeral=True
            )
            print(f"[ERRO] HTTP ao alterar apelido: {e}")
            ...
            print(f"[ERRO] Falha ao alterar apelido: {e}")
...
class ConfirmarCompraView(discord.ui.View):
    ...
    async def confirmar(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.selecionado == "cargo":
            ...
            try:
                if self.canal_temp:
                    await self.canal_temp.delete(reason="Conversa de compra encerrada (cargo VIP criado)")
            except Exception:
                pass
            return
        ...
        if self.selecionado == "boost_xp":
            ...
            role_name = f"⚡ BoostXP-{self.user.display_name}"
            try:
                role = await guild.create_role(name=role_name, color=discord.Color.gold(), reason="Boost XP ativo por 24h")
                try:
                ...
                role = None 
            try:
                asyncio.create_task(self._schedule_boost_end(interaction.client, self.user, guild, duration, role))
            except Exception:
                pass
            ...
            try:
                ...
                time_str = f"{rel} (até {abs_time})"
            except Exception:
                time_str = f"{duration // 3600} horas"
            ...
            try:
                if self.canal_temp:
                    await self.canal_temp.delete(reason="Conversa de compra encerrada (boost aplicado)")
            except Exception:
                pass
        ...
        try:
            if self.canal_temp:
                await self.canal_temp.delete(reason="Conversa de compra encerrada")
        except Exception:
            pass
    ...
    async def _schedule_boost_end(self, bot, user: discord.Member, guild: discord.Guild, delay: int, role: Optional[discord.Role] = None):
        try:
        ...
        try:
            if not db.check_boost_active(user.id, guild.id):
                return
        except Exception:
            pass
        ...
        if role:
            try:
                ...
                await role.delete(reason="Boost expirado")
            except Exception:
                pass
        else:
        ...
        try:
            await user.send("⏰ Seu **Boost de XP** terminou! Esperamos que tenha aproveitado o bônus. ⚡")
        except Exception:
            pass
